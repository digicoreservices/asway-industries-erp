...
import { UNIT_OPTIONS } from '@/hooks/useUnitOptions';

const EditPIForm = ({ item, onClose }) => {
  const { toast } = useToast();
  const { suppliers } = useSupplierManagement();
  const { updateItem } = useItemManagement();
  
  const [formData, setFormData] = useState({
    ...item,
    invoice_date: item.invoice_date ? parseISO(item.invoice_date) : null,
  });
  ...
  useEffect(() => {
    if (formData.category) {
      setItemNames(itemData[formData.category] || []);
    }
  }, [formData.category]);
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    const { id, item_name, category, unit, quantity, price, supplier_id } = formData;
    if (!item_name || !category || !unit || !quantity || !price || !supplier_id) {
      toast({
    ...
    const result = await updateItem(id, updatedItemData);
        ...
        <form onSubmit={handleSave}>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[70vh] overflow-y-auto pr-3">
            <div className="space-y-2">
              ...
              <Select onValueChange={(value) => handleSelectChange('supplier_id', value)} value={formData.supplier_id} required>
                <SelectTrigger><SelectValue placeholder="Select a supplier" /></SelectTrigger>
                <SelectContent className="max-h-[200px]">{suppliers.map(supplier => <SelectItem key={supplier.id} value={supplier.id}>{supplier.supplier_name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label htmlFor="supplier_invoice_number">Supplier Invoice Number</Label>
              <Input id="supplier_invoice_number" placeholder="Enter Invoice Number" value={formData.supplier_invoice_number} onChange={handleInputChange} />
            </div>
            <div className="space-y-2">
              <Label>Invoice Date</Label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !formData.invoice_date && "text-muted-foreground")}>
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {formData.invoice_date ? format(formData.invoice_date, "dd-MM-yyyy") : <span>Pick a date</span>}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0"><Calendar mode="single" selected={formData.invoice_date} onSelect={handleDateChange} initialFocus /></PopoverContent>
              </Popover>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Select onValueChange={(value) => handleSelectChange('item_name', value)} value={formData.item_name} disabled={!formData.category} required>
                <SelectTrigger><SelectValue placeholder="Select an item name" /></SelectTrigger>
                <SelectContent className="max-h-[200px]">{itemNames.map((name, index) => <SelectItem key={`${name}-${index}`} value={name}>{name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label>Unit</Label>
              <Select onValueChange={(value) => handleSelectChange('unit', value)} value={formData.unit} required>
                <SelectTrigger><SelectValue placeholder="Select a unit" /></SelectTrigger>
                <SelectContent className="max-h-[200px]">{UNIT_OPTIONS.map(unit => <SelectItem key={unit} value={unit}>{unit}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label htmlFor="quantity">Quantity</Label>
              <Input id="quantity" type="number" placeholder="Enter quantity" value={formData.quantity} onChange={handleInputChange} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="price">Price of One Unit</Label>
              <Input id="price" type="number" placeholder="Enter price" value={formData.price} onChange={handleInputChange} required />
            </div>
            <div className="space-y-2">
